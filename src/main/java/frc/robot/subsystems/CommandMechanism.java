...
public class CommandMechanism extends BaseMechanism{
    ...
    public CommandMechanism(Arm arm, Intake intake, Indexer indexer, Spindexer spindexer, Hood hood, Shooter shooter, ClimbElevator climbElevator, SwerveDrive swerveDrive){
        super(arm, intake, indexer, spindexer, hood, shooter, climbElevator, swerveDrive);
        ...
        if (Robot.isSimulation()){
            Bootleg2026.addShooterSimulation(
                ()->fromSwerveBase.plus(new Transform3d(0,0,0,new Rotation3d(0,Units.rotationsToRadians(.25-hoodRotationsToShootRotations(hood.getPosition())),0)))
                    ,()->shooter.getVelocity() * 1.07, "Fuel", "Intake");
            Bootleg2026.addShootRequirements("Intake", ()->{
                boolean ready = readyToShoot() && indexer.getVelocity() > 2 && Utils.getCurrentTimeSeconds() > lastLaunch + .15;
                if (ready){
    ...
    public boolean readyToShootHub(){
        return readyToShoot()
        && Math.abs(dynamicScoringData[2]-swerveDrive.getYaw().getRotations()) < maxAimError;
    }
    ...
    public Command stopShooting(){
        return Commands.parallel(shooter.reachGoalOnce(0), hood.reachGoalOnce(0), spindexer.reachGoalOnce(0), indexer.reachGoalOnce(0));
    }
    ...
    public Command passStatic(boolean left) {
        return Commands.parallel(swerveDrive.rotateTo(()->Rotation2d.fromRotations(left ? dynamicPassLeft[2] : dynamicPassRight[2]),5)
            ,shootDefault(()-> left ? dynamicPassLeft : dynamicPassRight, ()->readyToShoot()));
    }

    public Command passDynamic(BooleanSupplier left, DoubleSupplier vx, DoubleSupplier vy) {
        return shootDefault(()-> left.getAsBoolean() ? dynamicPassLeft : dynamicPassRight, ()->readyToShoot())
    ...
    public void commandPeriodic(){
        ...
        if (!Robot.isSimulation() && readyToShoot() && indexer.getVelocity() > 2 && Utils.getCurrentTimeSeconds() > lastLaunch + .2){
            lastLaunch = Utils.getCurrentTimeSeconds();
            animations.addFlyingObject(swerveDrive.getPose(), fromSwerveBase.getTranslation(), new Rotation3d(0,Units.rotationsToRadians(.25-hoodRotationsToShootRotations(hood.getPosition())),Units.rotationsToRadians(0)), swerveDrive.getSpeeds(), rpsTomps(shooter.getVelocity()));
        }
    }
}